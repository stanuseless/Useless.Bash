...
`0.4.6-UNSTABLE`
| [GitHub](https://github.com/stanuseless/Useless.Bash/releases/tag/0.4.6-UNSTABLE)
...
$ ./assemble.sh \
 && ./src/test/bash/unit_test.sh \
 && unzip -d /opt/Useless.Bash-0.4.6-UNSTABLE ./build/zip/Useless.Bash-0.4.6-UNSTABLE.zip
```
...
```
$ TMP_PATH="$(mktemp)"; \
 curl -L 'https://github.com/stanuseless/Useless.Bash/releases/download/0.4.6-UNSTABLE/Useless.Bash-0.4.6-UNSTABLE.zip' \
  -o "${TMP_PATH}" && unzip -d /opt/Useless.Bash-0.4.6-UNSTABLE "${TMP_PATH}" && rm "${TMP_PATH}"
```
